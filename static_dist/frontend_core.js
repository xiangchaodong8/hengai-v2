...
;(function (global) {
  // 防呆：API_BASE 默认 ""（同源）；勿用 || 覆盖空字符串
  if (global.API_BASE == null || global.API_BASE === undefined) {
    global.API_BASE = (global.APP_CONFIG && (global.APP_CONFIG.API_BASE || global.APP_CONFIG.api_base)) ?? ""
  }
  global.API_BASE = String(global.API_BASE).replace(/\/+$/, "").replace(/\/api\/v1$/i, "")
  global.HENGAI_API_BASE = global.API_BASE
  var API_BASE = global.API_BASE
  /** 身份校验唯一 URL（与后端契约一致），始终带 Bearer */
  global.API_AUTH_ME_URL = global.API_AUTH_ME_URL || (API_BASE + "/api/v1/auth/me")
  var API_AUTH_ME_URL = global.API_AUTH_ME_URL
  ...
  const FIELD_MAP = [
    ...
    { cls: "dyn-industry-bar",       path: "metrics.barWidthPct",        attr: "style.width", fn: (v) => `${Math.min(100, Number(v)||0).toFixed(1)}%` },

    // ── 🚨 任务 4B · 首页 ESG 战绩 / 影响力网络区块
    { cls: "dyn-carbon-intensity",   path: "metrics.carbonIntensity",    fn: (v) => v != null ? Number(v).toFixed(2) : "待测算" },
    { cls: "dyn-tax-savings-wan",    path: "metrics.taxSavingsWan",      fn: (v) => v ? Number(v).toFixed(1) : "--" },
    { cls: "dyn-roi-multiple",       path: "metrics.roiMultiple",        fn: (v) => v ? Number(v).toFixed(1) : "--" },
    { cls: "dyn-industry-better-pct",path: "metrics.industryBetterPct",  fn: (v) => v ? Number(v).toFixed(1) : "--" },
    { cls: "dyn-supplier-push",      path: "metrics.supplierPush",       fn: fmt.num },
    { cls: "dyn-cbam-drills",        path: "metrics.cbamDrills",         fn: fmt.num },
    { cls: "dyn-paid-partners",      path: "metrics.paidPartners",       fn: fmt.num },
    { cls: "dyn-coverage-delta-pct", path: "metrics.coverageDeltaPct",   fn: fmt.num },
    { cls: "dyn-gm-month-delta",     path: "metrics.gmMonthlyDelta",     fn: fmt.num },
  ...
  function syncAllInternalData(state) {
    ...
    // 批次 5：与 AppState 财务内核 / 会员身份 / iframe 管道对齐
    try {
      if (typeof window.formatHubUserIdentity === "function" && state.user) {
        const ident = window.formatHubUserIdentity(state.user)
        Object.assign(state.user, {
          tier_code: ident.tierCode,
          tier: ident.tierLabel,
          tierLabel: ident.tierLabel,
          regDate: ident.regDate || state.user.regDate,
          regLabel: ident.regLabel,
        })
        document.querySelectorAll(".dyn-user-tier").forEach((el) => { el.textContent = ident.tierLabel })
        document.querySelectorAll(".dyn-user-reg-date").forEach((el) => {
          el.textContent = ident.regDate || "—"
        })
      }
    } catch (idErr) {
      console.warn("[HengAI] formatHubUserIdentity", idErr)
    }
    try {
      if (typeof window.applyFinancialsInDocument === "function") {
        window.applyFinancialsInDocument(state)
      } else if (typeof window.applyRepFinancialsToDom === "function") {
        window.applyRepFinancialsToDom(state)
      }
    } catch (finErr) {
      console.warn("[HengAI] applyFinancials", finErr)
    }
    try {
      if (typeof window.broadcastHubPipelineToEmbeds === "function") {
        window.broadcastHubPipelineToEmbeds(state)
      }
    } catch (pipeErr) {
      console.warn("[HengAI] broadcastHubPipelineToEmbeds", pipeErr)
    }
  }
  ...
  function listenToStateUpdates() {
    ...
    window.addEventListener("message", (e) => {
      const d = e && e.data
      if (!d || (d.type !== "HENGAI_DATA_CHANGED" && d.type !== "DATA_SAVED")) return
      initHengAI().catch((err) => console.warn("[HengAI] embed refresh", err))
    })
    ...
    window.addEventListener("hengai:actions-taken", (e) => {
      const p = e && e.detail
      if (!p || p._skipApply) return
      if (p.updatedState && typeof window.hengaiApplyChatStateUpdate === "function") {
        window.hengaiApplyChatStateUpdate(p.updatedState, { source: "frontend_core" })
        return
      }
      if (typeof window.hengaiAfterStateSync === "function") {
        window.hengaiAfterStateSync(window.AppState, { source: "frontend_core-actions", light: true })
      }
    })
    ...
    window.addEventListener("hengai:phase-changed", (e) => {
      // 首页对话区禁止全屏「阶段跃迁」打断聊天；仅全域中心壳内庆祝
      const onIndexChat =
        document.getElementById("chatContainer") &&
        !document.getElementById("page-overview")
      if (onIndexChat) return
      _triggerPhaseUpgradeAnimation(e.detail && e.detail.newPhase)
    })
  }
   ...
   * initHengAI()
   *
   * 1. 尝试从 ${API_BASE}/api/v1/hub/overview 拉取并合并 AppState（非 2xx 则保留 MOCK/缓存骨架）
  ...
  async function initHengAI() {
    let initState = null
    let initError = null
    /** 与 AppState.initAppState 对齐：5xx/网络异常不算“引擎挂了”，只算离线骨架 */
    let mergedFromRemote = false

    try {
      if (!window.AppState) {
        throw new Error("window.AppState 未定义（请确认 AppState.js 先于 frontend_core.js 加载）")
      }

      const token = (function readAuthTokenSafe() {
        try {
          if (typeof global.getToken === "function") {
            const t = global.getToken()
            if (t) return t
          }
        } catch (tokenErr) {
          console.warn("[HengAI] getToken 调用异常，回退 localStorage", tokenErr)
        }
        try {
          return (
      ...
      let res = await fetch(`${API_BASE}/api/v1/hub/overview`, {
        credentials: "include",
        headers,
      })
      ...
      if (res.status === 401 && token) {
        try {
          if (typeof global.clearToken === "function") global.clearToken()
        } catch (_) {}
        delete headers.Authorization
        res = await fetch(`${API_BASE}/api/v1/hub/overview`, {
          credentials: "include",
          headers,
        })
      }
      ...
      if (!res.ok) {
        ...
        if (res.status !== 401) {
          console.warn(
            `[HengAI] hub/overview 不可用 HTTP ${res.status}，首屏使用本地骨架（与 initAppState 降级一致）`
          )
        }
      } else {
        const raw = await res.json().catch(() => ({}))
        const live =
          typeof window.sanitizeOverviewPayload === "function"
            ? window.sanitizeOverviewPayload(raw)
            : raw && typeof raw === "object" && !Array.isArray(raw)
              ? raw
              : {}
        const hasPayload = live && typeof live === "object" && Object.keys(live).length > 0
        if (!hasPayload) {
          console.warn("[HengAI] hub/overview 返回空载荷，保持本地骨架")
        } else {
          try {
            if (
              token &&
              typeof window.hubOverviewUserMatchesCurrent === "function" &&
              !window.hubOverviewUserMatchesCurrent(live)
            ) {
              console.warn("[HengAI] hub/overview user 与 JWT 不一致，丢弃防串号")
            } else if (window.AppState && typeof window.replaceAuthoritativeAppStateFromLive === "function") {
              window.replaceAuthoritativeAppStateFromLive(live)
            ...
            if (mergedFromRemote) {
              if (token && window.AppState.user && typeof window.hydrateAuthSession === "function") {
                try {
                  await window.hydrateAuthSession({ skipFetch: true })
                } catch (authErr) {
                  console.warn("[HengAI] overview 后 auth 同步失败", authErr)
                }
              } else if (token && window.AppState.user) {
                window.AppState.auth = window.AppState.auth || {}
                window.AppState.auth.user = window.AppState.user
                window.AppState.auth.token = token
              }
            }
          } catch (mergeErr) {
            mergedFromRemote = false
            console.warn("[HengAI] hub/overview 合并失败，保留本地骨架", mergeErr)
          }
        }
      }
      ...
      syncAllInternalData(window.AppState)
      try {
        if (typeof window.pulseHubAfterDataSync === "function") {
          window.pulseHubAfterDataSync(window.AppState)
        }
      } catch (pulseErr) {
        console.warn("[HengAI] pulseHubAfterDataSync", pulseErr)
      }
      ...
      if (mergedFromRemote) {
        console.info(
          `[HengAI] ✅ AppState 初始化完成（在线） | API_BASE=${API_BASE} | Phase=${initState.flags && initState.flags.currentPhase} | GM=${initState.user && initState.user.gmBalance}`
        )
      } else {
        console.info(
          `[HengAI] AppState 就绪（离线骨架） | API_BASE=${API_BASE} — 待后端恢复后由 initAppState / 刷新自动对齐`
        )
      }
    } catch (err) {
      initError = err
      ...
      try {
        if (typeof global.syncAppState === "function" && window.AppState) {
          global.syncAppState(window.AppState, { fromRemote: mergedFromRemote })
        }
      } catch (_) {}
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent("hengai:init-complete", {
          detail: {
            ok: !initError,
            degraded: Boolean(!initError && !mergedFromRemote),
            state: initState || window.AppState,
            error: initError,
          },
        }))
      } catch (broadcastErr) {
  ...
  function wireCoreStateSynced() {
    if (wireCoreStateSynced._done) return
    wireCoreStateSynced._done = true
    if (typeof window.EventBus === "undefined" || !window.EventBus.on) return
    window.EventBus.on("STATE_SYNCED", (state) => {
      const s = state || window.AppState
      if (typeof syncAllInternalData === "function") syncAllInternalData(s)
    })
  }
  wireCoreStateSynced()

  /**
   * 统一身份校验：GET `${API_BASE}/api/v1/auth/me`，必须带 Bearer（无 token 时不请求）
   */
  async function fetchAuthMe() {
    ...
    let res
    try {
      res = await fetch(API_AUTH_ME_URL, {
        credentials: "include",
        headers: { Accept: "application/json", Authorization: "Bearer " + token },
      })
    } catch (e) {
      console.warn("[HengAI] auth/me 网络异常", e)
      return { ok: false, user: null }
    }
    if (!res.ok) return { ok: false, status: res.status, user: null }
    const data = await res.json().catch(() => ({}))
    const userObj = data?.user || data?.data || data
    const user = userObj && typeof userObj === "object" && !Array.isArray(userObj) ? userObj : null
